...
class BuggetSupabaseUser extends BaseAuthUser {
  BuggetSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> buggetSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BuggetSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
